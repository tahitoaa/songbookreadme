...
function main(argv)
  ...
  while argv[i] do
    ...
    elseif argv[i] == "-h" or argv[i] == "--help" then
      io.write("Syntax: ",argv[-1]," ",argv[0]," [options] input.sxd [output.sbx]\n",
"Available options:\n",
...
"See http://songs.sourceforge.net for support.\n")
      return 0
    elseif argv[i] == "-b" or argv[i] == "--bible" then
      if biblename then
      ...
      if argv[i] then
        ...
        io.stderr:write("songidx: ",argv[i-1]," option requires an argvument\n")
        return 2
      end
    elseif argv[i] == "-l" or argv[i] == "--locale" then
      if locale then
      ...
      if argv[i] then
        ...
        io.stderr:write("songidx: ",argv[i-1]," requires an argvument\n")
        return 2
      end
    elseif argv[i] == "-o" or argv[i] == "--output" then
      if outname then
    ...
    elseif argv[i]:sub(1,1) == "-" and argv[i] ~= "-" then
      io.stderr:write("songidx: unknown option ",argv[i],"\n")
      return 2
    elseif not inname then inname = argv[i]
    elseif not outname then outname = argv[i]
    else
      io.stderr:write("songidx: too many command line argvuments\n")
      return 2
    end
    i = i + 1
  end
...
function geneidx.makeindexfile(filename)
    main(indexargs(filename))
    local titlfile = assert(io.popen('titlfile.sbx', 'r')) 
    local str = fout:read('*a')
    local fout = io.popen('tmp.txt', 'w')
    fout:write(str)
    fout:close()
    titlfile:close()
end
